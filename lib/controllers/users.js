...
exports.getAcceptedFriends = function(req, res) {

  var Status = require("mongoose-friends").Status;
  User.getFriends(req.user, {"friends.status": Status.Pending}, function (err, friendships) {
    if (err) return res.send(400);
    return res.json(friendships);
  });
};
...
exports.findByNameOrEmail = function(req, res) {
  if(req.params.nameOrEmail.length > 1){
    //name search
    if(req.params.nameOrEmail.indexOf("@") === -1) {
      var blacklist = /[\.\[\]]/;
      if(req.params.nameOrEmail.search(blacklist) === -1) {
        var regex = new RegExp("^" + req.params.nameOrEmail, "i");
        var query = User.find({"name" : regex }).limit(10);
