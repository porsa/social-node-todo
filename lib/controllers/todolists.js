...
exports.deleteOneTodoItem = function (req, res) {
  TodoList.findOne({"_id": new ObjectId(req.params.id)}, function(err, todoList) {
      ...
      var items = todoList.todoItems.filter(function(item) {
        if (item._id !== null) {
          return (String(item._id) === req.body.todoItemId);
        }
        return false;
      });

      if(items.length === 1) {
        todoList.todoItems.remove(items[0]);
      }
